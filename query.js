...
router.get('/tagihanUpdate', function(req, res){
  ...
  var sql = "update tagihan set status = ?, tgl_bayar = ? where NCLI = ? and status = ?";
  var id = req.query.id;
  var status = req.query.status;
  var oldStatus = req.query.oldStatus;
  var now = moment().format();
  connection.query(sql,[status,now, id, oldStatus], function(err, rows, fields){
    if(err) throw err;
...
router.get('/history', function(req, res){
  var sql = "select a.NCLI, a.NAMA, sum(B.HARGA) AS TOTAL, b.TGL_BAYAR, b.STATUS from tagihan b left join pelanggan a on a.NCLI = b.NCLI where b.status != 0 group by a.NCLI, b.STATUS order by b.TGL_BAYAR desc"
  connection.query(sql, function(err, rows, fields){
    if(err) throw err;
